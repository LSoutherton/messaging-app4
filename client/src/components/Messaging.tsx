...
const Messaging = ({ user }) => {
  ...
  const getMessages = async () => {
    try {
      const response = await UserFinder.get(`/getConversation/${userConversation}/${currentUser.username}`)
      ...
      {screenSize ?
          ...
          <div className='min-h-screen'>
            <div className=''>
              <div className='pt-2 pb-2 grid grid-cols-3 bg-gray-200 border-b-2 border-orange-500 mb-2 sticky top-0 z-1000 p-4 h-16'>
                <Link to='/main'>
                  <ChevronLeftIcon className='float-left ml-4 mt-2 text-orange-500' fontSize='large'/>
                </Link>
                <h1 className='text-3xl text-orange-500 font-semibold flex m-auto'>{userConversation}</h1>
              </div>
        ...
        <div className='min-h-screen'>
            ...
            <div className='pt-2 pb-2 text-center bg-gray-200 border-b-2 border-orange-500 mb-2 sticky top-0 z-1000 p-4 h-16'>
                <h1 className='text-3xl text-orange-500 font-semibold mt-1'>{userConversation}</h1>
            </div>
            <div className='overflow-scroll h-fit'>
                {renderMessages}
            </div>
